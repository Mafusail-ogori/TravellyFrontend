    ...
    return <ContentWrapper color="#FFCA42" whiteText="ДОДАВАЙ" blackText="СТВОРЮЙ" height="1600px">
        <FormCard onSubmit={addOrEditTrip}>
            ...
            <div className={styles.container}>
                <RowerWrapper>
                    <Input name = "name"  placeholder="Назва путівки " type="text" label="Назва путівки "
                           height="50px" status={status} />
                    <Input name = "amount" placeholder="Максимальна кількість людей " type="number"
                           label="Максимальна кількість людей "
                           height="50px" status={status} />
                    <Select name = "animal"  label="Навність тварин " height="50px" status={status}
                            options={boolean}/>
                    <Select name = "transfer"  label="Трансфер " height="50px" status={status}
                            options={transfer}/>
                    <Select name = "food"  label="Харчування " height="50px" status={status}
                            options={food}/>
                    <Input name = "hotel"  placeholder="Назва готелю " type="text" label="Назва готелю "
                           height="50px" status={status} />
                </RowerWrapper>
                <RowerWrapper>
                    <Select name = "startCountry" label="Країна відправки " height="50px" status={status}
                            options={countryList} />
                    <Select name = "endCountry"  label="Країна прибуття " height="50px" status={status}
                            options={countryList}/>
                    <Input name = "startDate"  type='date' label="Початок подорожі " height="50px"
                           status={status} />
                    <Input name = "endDate"  type='date' label="Кінець подорожі " height="50px"
                           status={status} />
                    <Input name = "image"  type='file' label="Фото путівки " height="50px" status={status}/>
                    <Input name = "price"  placeholder='Ціна за людину' type='number'
                           label="Ціна за людину " height="50px"
                           status={status} />
                </RowerWrapper>
            </div>
            <TextArea name = "description" height = "100px" label = "Опис" placeholder = "Опис" fontSize = "14px" status = {status}
            />
